        ...
        >
          ...
          >
            {visibleTimeline.map((day) => {
              ...
              const displaySlots = showFlyableOnly
                ? enrichedSlots.filter((slot) => slot.isFlyable)
                : enrichedSlots;
              const flyableCount = enrichedSlots.filter(
                (slot) => slot.isFlyable,
              ).length;
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-[10px] text-slate-500">
                        {day.slots.length} חלונות זמן · {flyableCount} שעות
                        יציבות
                      </div>
                      {firstFlyable && (
                      ...
                      displaySlots.map((slot) => {
                        ...
                        return (
                          <button
                            key={slot.key}
                            onClick={() =>
                              onSlotSelect(`${day.day}T${slot.time}`)
                            }
