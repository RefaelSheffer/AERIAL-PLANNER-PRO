...
const WEATHER_ONLY_MODE = !ENABLE_MISSION_PLANNING && !ENABLE_REALTIME_PANEL && !ENABLE_DOCUMENTATION;
...
const isLocalHostname = (hostname) => hostname === "localhost" || hostname === "127.0.0.1" || hostname === "[::1]" || hostname.endsWith(".local");
const loadLocalEnvOverrides = async () => {
  if (!isLocalHostname(window.location.hostname)) return false;
  const basePath = (window.APP_ENV?.APP_BASE_PATH || window.AERIAL_PLANNER_ENV?.APP_BASE_PATH || "").replace(/\/$/, "");
...
(async () => {
  ...
  const slotIsFlyable = (slot, suitabilitySettings) => {
    ...
    const safeGust = gust === null ? wind === null ? true : wind <= maxGust : gust <= maxGust;
    const safeCloud = clouds === null ? true : clouds >= minCloudCover && clouds <= maxCloudCover;
    const safeRain = rainProb === null ? true : rainProb <= maxRainProb;
    const sunOk = includeNightFlights ? true : slot.sunAlt === null ? true : slot.sunAlt >= minSunAltitude && slot.sunAlt <= maxSunAltitude;
  ...
  const calculateSlotRisk = (slot, suitabilitySettings) => {
    ...
    const rainProb = typeof slot.rainProb === "number" && !Number.isNaN(slot.rainProb) ? slot.rainProb : null;
    const safeDivisor = (value, fallback = 1) => typeof value === "number" && value > 0 ? value : fallback;
    const windRisk = wind === null ? 0 : Math.max(0, (wind - maxWind) / safeDivisor(maxWind));
    const gustRisk = gust === null ? 0 : Math.max(0, (gust - maxGust) / safeDivisor(maxGust));
    const rainDenominator = Math.max(1, 100 - maxRainProb);
    const rainRisk = rainProb === null ? 0 : Math.max(0, (rainProb - maxRainProb) / rainDenominator);
    const sunRisk = includeNightFlights ? 0 : slot.sunAlt === null || slot.sunAlt === void 0 ? 0 : slot.sunAlt < minSunAltitude || slot.sunAlt > maxSunAltitude ? 1 : 0;
    const riskComponents = [windRisk, gustRisk, rainRisk, sunRisk];
    const baseScore = riskComponents.reduce((total, value) => total + value, 0) / 4;
  ...
  const buildWindTimeline = (hourlyForecast, weatherLocation) => {
    ...
    hourly.time.forEach((t, i) => {
      const slotDate = /* @__PURE__ */ new Date(`${t}Z`);
  ...
  const App = () => {
    ...
    const [flightDate, setFlightDate] = useState(
      (/* @__PURE__ */ new Date()).toISOString().slice(0, 16)
    );
    ...
    const [windUnit, setWindUnit] = useState(() => {
      ...
      return WIND_UNITS.some((unit) => unit.id === saved) ? saved : WIND_UNITS[0].id;
    });
    ...
    const [pushSubscriptionId, setPushSubscriptionId] = useState(() => {
      if (typeof window === "undefined") return null;
      try {
        return window.localStorage.getItem(PUSH_SUBSCRIPTION_ID_STORAGE_KEY) || null;
      } catch (e) {
        return null;
      }
    });
    ...
    const convertWindToDisplay = useCallback(
      (value) => typeof value === "number" && !Number.isNaN(value) ? value * windUnitMeta.factor : null,
      [windUnitMeta]
    );
    const convertWindToBase = useCallback(
      (value) => typeof value === "number" && !Number.isNaN(value) ? value / windUnitMeta.factor : 0,
      [windUnitMeta]
    );
    ...
    const [dtmData, setDtmData] = useState(
      () => ENABLE_MISSION_PLANNING ? null : void 0
    );
    ...
    const [aircraftTimestamp, setAircraftTimestamp] = useState(
      () => ENABLE_REALTIME_PANEL ? null : void 0
    );
    ...
    const ALLOWED_DOC_IMAGE_TYPES = useMemo(
      () => /* @__PURE__ */ new Set(["image/jpeg", "image/png", "image/webp", "image/gif"]),
      []
    );
    const MAX_DOC_IMAGE_BYTES = 5 * 1024 * 1024;
    ...
    const opticalZoomLevels = useMemo(() => {
      ...
      return activeDrone.opticalZoomLevels.length > 0 ? activeDrone.opticalZoomLevels : null;
    }, [activeDrone]);
    ...
    useEffect(() => {
      ...
      const localEntries = typeof localStorage !== "undefined" ? loadEntriesFromStorage(localStorage, DOC_LOCAL_STORAGE_KEY) : null;
      if (localEntries?.length) {
        setDocEntries(localEntries);
        setDocStorageMode("local");
        return;
      }
      const sessionEntries = typeof sessionStorage !== "undefined" ? loadEntriesFromStorage(sessionStorage, DOC_STORAGE_KEY) : null;
    ...
    useEffect(() => {
      if (!ENABLE_DOCUMENTATION) return;
      if (typeof window === "undefined") return;
      const targetKey = docStorageMode === "local" ? DOC_LOCAL_STORAGE_KEY : DOC_STORAGE_KEY;
      const targetStorage = docStorageMode === "local" ? localStorage : sessionStorage;
      try {
      ...
      const staleKey = docStorageMode === "local" ? DOC_STORAGE_KEY : DOC_LOCAL_STORAGE_KEY;
      const staleStorage = docStorageMode === "local" ? sessionStorage : localStorage;
      try {
    ...
    const themeStyles = useMemo(() => {
      ...
      return {
        panel: isDark ? "bg-slate-900 text-white" : "bg-white text-slate-900 border-l border-slate-200",
        header: isDark ? "bg-slate-800 border-b border-slate-700" : "bg-slate-50 border-b border-slate-200",
        tabList: isDark ? "bg-slate-900/60 border border-slate-700" : "bg-white border border-slate-200 shadow-sm",
        tabActive: "bg-blue-600 text-white",
        tabInactive: isDark ? "text-slate-200 hover:bg-slate-700/80" : "text-slate-700 hover:bg-blue-50",
        iconButton: isDark ? "bg-slate-700 p-2 rounded hover:bg-slate-600 text-white" : "bg-white p-2 rounded border border-slate-200 hover:bg-slate-50 text-slate-700 shadow-sm",
        closeButton: isDark ? "bg-slate-700 p-2 rounded-full hover:bg-slate-600" : "bg-white p-2 rounded-full border border-slate-200 hover:bg-slate-50 text-slate-700 shadow-sm",
        card: isDark ? "bg-slate-800 border border-slate-700" : "bg-white border border-slate-200 shadow-sm",
        subtleText: isDark ? "text-slate-400" : "text-slate-600",
        select: "w-full p-2 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500",
        selectTone: isDark ? "bg-slate-900 border border-slate-700 text-white" : "bg-white border border-slate-300 text-slate-900",
    ...
    const settingsTheme = useMemo(() => {
      ...
      return {
        ...
        closeButton: isDark ? "border-slate-700 bg-slate-800 text-slate-200 hover:bg-slate-700 hover:text-white" : "border-slate-200 bg-white/95 text-slate-600 hover:bg-slate-100 hover:text-slate-900",
        ...
        summary: isDark ? "text-slate-300 bg-slate-800 border-slate-700" : "text-slate-600 bg-slate-50 border-slate-200",
        input: isDark ? "bg-slate-800 text-slate-100 border-slate-700 placeholder-slate-500 focus:ring-blue-400" : "",
        panel: isDark ? "border-slate-700 bg-slate-800" : "border-slate-200 bg-white"
      };
    }, [theme]);
    const vapidPublicKey = Config.VAPID_PUBLIC_KEY;
    const supabaseAnonKey = Config.SUPABASE_ANON_KEY;
    const appBasePath = (Config.APP_BASE_PATH || "").replace(/\/$/, "");
    const subscribeEndpoint = "https://wpmxaadzsxxyyhhpsywf.supabase.co/functions/v1/subscribe";
    ...
    useEffect(() => {
      ...
      navigator.serviceWorker.getRegistration(scope).then(
        (registration) => registration ? registration.pushManager.getSubscription() : null
      ).then((subscription) => {
        if (!isActive) return;
        if (subscription) {
    ...
    const showPlannerLayout = ENABLE_MISSION_PLANNING || ENABLE_REALTIME_PANEL || ENABLE_DOCUMENTATION;
    ...
    useEffect(() => {
      ...
      refreshRainRadar();
      radarIntervalRef.current = setInterval(refreshRainRadar, 5 * 60 * 1e3);
      return () => {
        if (radarIntervalRef.current) {
          clearInterval(radarIntervalRef.current);
          radarIntervalRef.current = null;
    ...
    useEffect(() => {
      ...
      fetchAircraft();
      aircraftIntervalRef.current = setInterval(fetchAircraft, 15e3);
      return () => {
        if (aircraftIntervalRef.current) {
          clearInterval(aircraftIntervalRef.current);
          aircraftIntervalRef.current = null;
    ...
    const isSlotRelevant = useCallback(
      (slot) => {
        ...
        if (slot.sunAlt === null || slot.sunAlt === void 0) return true;
        return slot.sunAlt >= minSunAltitude && slot.sunAlt <= maxSunAltitude;
      },
      [suitabilitySettings]
    );
    ...
    const daySuitability = useMemo(() => {
      const formatWindRange = (values) => {
        ...
        const converted = values.map((value) => convertWindToDisplay(value)).filter((value) => typeof value === "number" && !Number.isNaN(value));
      ...
      const getWindows = (slots) => {
        ...
        return windows.map((window2) => {
          const startTime = window2.start.time;
          const endDate = /* @__PURE__ */ new Date(`${window2.end.key.slice(0, 13)}:00`);
          endDate.setHours(endDate.getHours() + SLOT_HOURS);
          const endTime = endDate.toISOString().slice(11, 16);
          return {
            label: `${startTime}–${endTime}`,
            hours: window2.count * SLOT_HOURS,
            count: window2.count
          };
        });
      };
      ...
      return windTimeline.map((day) => {
        ...
        const percent = relevantEnriched.length > 0 ? Math.round(flyableSlots.length / relevantEnriched.length * 100) : 0;
        const baseRiskScore = relevantEnriched.length > 0 ? relevantEnriched.reduce(
          (total, slot) => total + calculateSlotRisk(slot, suitabilitySettings),
          0
        ) / relevantEnriched.length : 0;
        const availabilityPenalty = 1 - percent / 100;
        const dayRiskScore = percent > 0 ? (baseRiskScore + availabilityPenalty) / 2 : baseRiskScore;
        const winds = relevantSlots.map((slot) => slot.wind).filter((value) => typeof value === "number");
        const gusts = relevantSlots.map((slot) => slot.gust ?? slot.wind).filter((value) => typeof value === "number");
        const rain = relevantSlots.map((slot) => slot.rainProb).filter((value) => typeof value === "number");
        const clouds = relevantSlots.map((slot) => slot.clouds).filter((value) => typeof value === "number");
        const flyableWindows = getWindows(relevantEnriched);
        const windowHours = flyableWindows.map((window2) => window2.hours);
        ...
        return {
          ...
          flyableWindows: flyableWindows.map((window2) => window2.label),
    ...
    const shareDocumentationProject = async () => {
      ...
      const summaryText = [
        "שיתוף תיעוד משימה:",
        ...docEntries.slice(0, 5).map(
          (entry, idx) => `${idx + 1}. ${entry.title || "ללא כותרת"}${entry.notes ? ` – ${entry.notes.slice(0, 80)}` : ""}`
        ),
    ...
    const exportDocumentationShapefile = () => {
      ...
      const features = docEntries.filter((e) => e.location).map((e) => ({
        type: "Feature",
        geometry: {
          type: "Point",
    ...
    const computeSidebarWidthPx = useCallback(() => {
      ...
      if (trimmed.endsWith("%")) {
        ...
        return Number.isFinite(percent) ? Math.min(percent / 100 * window.innerWidth, 420) : 0;
      }
    ...
    const dockPositionClasses = useMemo(() => {
      return isMobile ? "flex justify-end" : "flex left-auto items-start justify-start";
    }, [isMobile]);
    const dockPositionStyle = useMemo(() => {
      if (isMobile) {
        const mobileGapPx = 12;
        const sidebarWidthPx = computeSidebarWidthPx();
        const sidebarOffsetPx = sidebarWidthPx > 0 ? sidebarWidthPx + mobileGapPx : mobileGapPx;
        const realtimeOffsetPx = realtimePanelOpen && realtimePanelWidth > 0 ? realtimePanelWidth + mobileGapPx : 0;
        const offsetPx = realtimeOffsetPx ? Math.max(realtimeOffsetPx, sidebarOpen ? sidebarOffsetPx : mobileGapPx) : sidebarOpen ? sidebarOffsetPx : mobileGapPx;
    ...
    const processDTM = (grid, isSim) => {
      ...
      const stats2 = computeElevationStats(grid);
      const summary = stats2 && grid.length ? { min: stats2.min, max: stats2.max, avg: stats2.sum / grid.length } : null;
      setDtmStats(summary);
    ...
    const calcShadows = (grid, minEle) => {
      ...
      const shadows = [];
      const referenceMin = typeof minEle === "number" ? minEle : Math.min(...grid.map((p) => p.ele));
      grid.forEach((pt) => {
    ...
    const generatePath = () => {
      ...
      const rotatedPoly = polyPoints.map(
        (p) => AerialPlanner.geometry.rotatePoint(p, -angle, center)
      );
    ...
    useEffect(() => {
      ...
      if (ENABLE_REALTIME_PANEL && aircraftEnabled && aircraftList.length > 0) {
        const markers = aircraftList.map((a) => {
          ...
          const speed2 = a.Spd ? `${Math.round(a.Spd)} kt` : "N/A";
          const altitude2 = a.Alt ? `${a.Alt} ft` : "N/A";
          const callsign = a.Call || a.Reg || a.Icao || "לא ידוע";
          marker.bindPopup(`
                    <div class="text-[12px]">
                        <div class="font-bold text-slate-800">${callsign}</div>
                        <div class="text-slate-700">גובה: ${altitude2}</div>
                        <div class="text-slate-700">מהירות: ${speed2}</div>
    ...
    useEffect(() => {
      ...
      if (userAccuracy !== null && userAccuracy !== void 0) {
    ...
    const stats = useMemo(() => {
      ...
      const gsd = d.sensorWidth * altitude * 100 / (activeFocalLength * d.imageWidth);
      ...
      const polygonAreaSqM = (() => {
        ...
        const meterPerDegLng = Math.PI / 180 * 6378137 * Math.cos(refLatRad);
      ...
      const coverageKm2 = polygonAreaSqM > 0 ? (polygonAreaSqM / 1e6).toFixed(2) : "0";
      return {
        gsd: gsd.toFixed(2),
        time: timeMin,
        images: imgCount,
    ...
    const renderWindUnitToggle = (className = "") => /* @__PURE__ */ React.createElement(
      "button",
      {
        type: "button",
    ...
    const timelineBoard = /* @__PURE__ */ React.createElement(
      TimelineBoard,
      {
        ...
        filterFlyableOnly,
        onToggleFilterFlyable: () => setFilterFlyableOnly((prev) => !prev),
    ...
    return /* @__PURE__ */ React.createElement(React.Fragment, null, toastItems.length > 0 && /* @__PURE__ */ React.createElement("div", { className: "fixed top-4 right-4 z-[2100] flex flex-col gap-2" }, toastItems.map((toast) => {
    ...
    })), showSettings && /* @__PURE__ */ React.createElement("div", { className: "fixed inset-0 z-[2000] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" }, /* @__PURE__ */ React.createElement(
      "div",
      {
      ...
      /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-4" }, /* @__PURE__ */ React.createElement("div", { className: "space-y-1" }, /* @__PURE__ */ React.createElement("label", { className: `text-sm font-semibold ${settingsTheme.label}` }, "מקסימום רוח ", renderWindUnitToggle("align-middle")), /* @__PURE__ */ React.createElement(
        ...
        "input",
        {
          ...
          onChange: (e) => updateSuitabilitySetting(
            "minCloudCover",
      ...
      timelineBoard
    ) : /* @__PURE__ */ React.createElement("div", { className: "flex flex-col md:flex-row h-full relative overflow-x-hidden" }, ENABLE_MISSION_PLANNING && /* @__PURE__ */ React.createElement(
      Sidebar,
      {
        open: sidebarOpen,
      ...
      /* @__PURE__ */ React.createElement("div", { className: `${themeStyles.header} p-4 sticky top-0 z-30 flex justify-between items-center gap-2` }, /* @__PURE__ */ React.createElement("h1", { className: "font-black text-lg tracking-wider text-blue-400" }, "SMART PLANNER"), /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-2" }, /* @__PURE__ */ React.createElement(
        "div",
        {
          role: "tablist",
        ...
        /* @__PURE__ */ React.createElement(
          "button",
          {
            ...
            className: `px-3 py-2 text-xs font-semibold transition focus:outline-none ${activeSidebarTab === "mission" ? themeStyles.tabActive : themeStyles.tabInactive}`,
            onClick: () => {
              setActiveSidebarTab("mission");
        ...
        /* @__PURE__ */ React.createElement(
          "button",
          {
            ...
            className: `px-3 py-2 text-xs font-semibold transition focus:outline-none border-r last:border-r-0 ${activeSidebarTab === "settings" ? themeStyles.tabActive : themeStyles.tabInactive} ${theme === "dark" ? "border-slate-700" : "border-slate-200"}`,
            onClick: () => {
              setActiveSidebarTab("settings");
              setShowSettings(true);
            }
      ...
      ), /* @__PURE__ */ React.createElement("button", { className: themeStyles.iconButton }, /* @__PURE__ */ React.createElement(Icon, { name: "gps" })), /* @__PURE__ */ React.createElement(
        "button",
        {
      ...
      /* @__PURE__ */ React.createElement("div", { className: "p-4 space-y-6 flex-1 pb-24 md:pb-6" }, /* @__PURE__ */ React.createElement("div", { className: "space-y-4" }, /* @__PURE__ */ React.createElement("div", { className: `${themeStyles.card} rounded-lg p-3 space-y-2` }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between gap-2" }, /* @__PURE__ */ React.createElement("div", { className: `flex items-center gap-2 text-sm font-semibold ${themeStyles.accent}` }, /* @__PURE__ */ React.createElement(Icon, { name: "drone", size: 18 }), " בחר רחפן מבצעי"), /* @__PURE__ */ React.createElement("div", { className: `flex items-center gap-2 text-[11px] ${themeStyles.subtleText}` }, /* @__PURE__ */ React.createElement("span", null, Object.keys(AerialPlanner.config.DRONE_PRESETS).length, " ", "דגמים זמינים"), /* @__PURE__ */ React.createElement(
        "button",
        {
          className: `px-2 py-1 rounded ${theme === "dark" ? "bg-slate-700 hover:bg-slate-600 text-slate-200" : "bg-blue-50 text-blue-700 border border-blue-100 hover:bg-blue-100"}`,
          onClick: () => setDronePanelOpen((o) => !o)
        },
        dronePanelOpen ? "הסתר" : "הצג"
      ))), dronePanelOpen ? /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("p", { className: `text-xs ${themeStyles.subtleText}` }, "בחר דגם כדי לראות חיישן, סוג משימה ולקבל חישובי טיסה מדויקים."), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-1 sm:grid-cols-2 gap-2" }, Object.entries(AerialPlanner.config.DRONE_PRESETS).map(
        ([k, v]) => /* @__PURE__ */ React.createElement(
          "button",
          {
            key: k,
            onClick: () => setSelectedDrone(k),
            className: `w-full text-right border rounded-lg px-3 py-2 transition shadow-sm hover:border-blue-400 hover:shadow-lg focus:outline-none ${selectedDrone === k ? "border-blue-500 bg-blue-500/10 text-blue-900" : theme === "dark" ? "border-slate-700 bg-slate-900 text-slate-100" : "border-slate-200 bg-white text-slate-900"}`
          },
          /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between" }, /* @__PURE__ */ React.createElement("span", { className: "font-semibold text-sm" }, v.name), /* @__PURE__ */ React.createElement(
            "span",
            {
              className: `text-[11px] px-2 py-1 rounded-full ${selectedDrone === k ? "bg-blue-600/20 text-blue-800" : themeStyles.chip}`
            },
            v.type
          )),
          /* @__PURE__ */ React.createElement("div", { className: `text-[11px] ${themeStyles.subtleText} mt-1` }, "חיישן ", v.sensorWidth, "x", v.sensorHeight, ' מ"מ · מוקד', " ", v.focalLength, ' מ"מ'),
          v.opticalZoomLevels?.length ? /* @__PURE__ */ React.createElement("div", { className: `text-[10px] ${themeStyles.accent} mt-1` }, "זום אופטי עד ", Math.max(...v.opticalZoomLevels), "x") : null
        )
      ))) : /* @__PURE__ */ React.createElement("div", { className: "space-y-2" }, /* @__PURE__ */ React.createElement("p", { className: `text-xs ${themeStyles.subtleText}` }, "פתיחה מלאה מציגה את כל הדגמים; ניתן לבחור מהרשימה המהירה."), /* @__PURE__ */ React.createElement(
        "select",
        {
          value: selectedDrone,
        ...
        Object.entries(AerialPlanner.config.DRONE_PRESETS).map(
          ([k, v]) => /* @__PURE__ */ React.createElement("option", { key: k, value: k }, v.name, " • ", v.type)
        )
      ))), hasOpticalZoom && opticalZoomLevels && /* @__PURE__ */ React.createElement("div", { className: `${themeStyles.card} rounded-lg p-3 mb-2` }, /* @__PURE__ */ React.createElement("div", { className: "flex justify-between text-xs mb-1" }, /* @__PURE__ */ React.createElement("span", null, "זום אופטי"), /* @__PURE__ */ React.createElement("span", { className: themeStyles.accent }, currentOpticalZoom, "x · ", activeFocalLength.toFixed(1), ' מ"מ')), /* @__PURE__ */ React.createElement(
        "input",
        {
          type: "range",
          min: "0",
          ...
          onChange: (e) => setOpticalZoomIndex(Number(e.target.value)),
          className: "w-full"
        }
      ), /* @__PURE__ */ React.createElement("div", { className: `flex justify-between text-[10px] ${themeStyles.subtleText} mt-1` }, opticalZoomLevels.map((level, idx) => /* @__PURE__ */ React.createElement(
        "span",
        {
          key: `${level}-${idx}`,
          className: idx === clampedOpticalZoomIndex ? "text-blue-300 font-semibold" : ""
        },
        level,
        "x"
      )))), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { className: "flex justify-between text-xs mb-1" }, /* @__PURE__ */ React.createElement("span", null, "גובה (AGL)"), /* @__PURE__ */ React.createElement("span", { className: "text-blue-400" }, altitude, "m")), /* @__PURE__ */ React.createElement(
        "input",
        {
          type: "range",
          min: "20",
      ...
      )), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { className: "flex justify-between text-xs mb-1" }, /* @__PURE__ */ React.createElement("span", null, "מהירות טיסה"), /* @__PURE__ */ React.createElement("span", { className: "text-blue-400" }, speed, " מ/ש")), /* @__PURE__ */ React.createElement(
        "input",
        {
          type: "range",
          min: "2",
      ...
      )), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { className: "flex justify-between text-xs mb-1" }, /* @__PURE__ */ React.createElement("span", null, "חפיפה קדמית"), /* @__PURE__ */ React.createElement("span", { className: "text-blue-400" }, overlapFront, "%")), /* @__PURE__ */ React.createElement(
        "input",
        {
          type: "range",
          min: "10",
      ...
      )), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { className: "flex justify-between text-xs mb-1" }, /* @__PURE__ */ React.createElement("span", null, "חפיפה צידית"), /* @__PURE__ */ React.createElement("span", { className: "text-blue-400" }, overlapSide, "%")), /* @__PURE__ */ React.createElement(
        "input",
        {
          type: "range",
          min: "10",
      ...
      )), /* @__PURE__ */ React.createElement("div", { className: `${themeStyles.card} p-2 rounded` }, /* @__PURE__ */ React.createElement("div", { className: "flex justify-between items-center mb-2" }, /* @__PURE__ */ React.createElement("span", { className: `text-xs font-bold flex items-center gap-1 ${themeStyles.subtleText}` }, /* @__PURE__ */ React.createElement(Icon, { name: "rotate" }), " כיוון טיסה"), /* @__PURE__ */ React.createElement("label", { className: `text-[10px] flex items-center gap-1 cursor-pointer ${themeStyles.subtleText}` }, /* @__PURE__ */ React.createElement(
        "input",
        {
          type: "checkbox",
          checked: autoOrient,
      ...
      ), " ", "אוטומטי")), /* @__PURE__ */ React.createElement(
        "input",
        {
          type: "range",
          min: "0",
      ...
      ))), /* @__PURE__ */ React.createElement("div", { className: `pt-4 border-t ${theme === "dark" ? "border-slate-700" : "border-slate-200"}` }, /* @__PURE__ */ React.createElement("div", { className: `${themeStyles.card} text-[11px] ${themeStyles.subtleText} rounded p-2 mb-2` }, "בחירת המועד נעשית מלוח מזג האוויר בתחתית. בחר שעה רצויה בלוח כדי לחשב צל בהתאם."), /* @__PURE__ */ React.createElement(
        "button",
        {
          onClick: fetchDTM,
      ...
      ), dtmStats && /* @__PURE__ */ React.createElement(
        "div",
        {
        ...
        isSimulatedDTM && /* @__PURE__ */ React.createElement("div", { className: "flex justify-center items-center gap-1 mb-1 font-bold" }, /* @__PURE__ */ React.createElement(Icon, { name: "warning" }), " סימולציית שטח (Offline Mode)"),
        "גבהים: ",
        dtmStats.min.toFixed(0),
        "m - ",
        dtmStats.max.toFixed(0),
        "m"
      ))),
      /* @__PURE__ */ React.createElement(
        "div",
        {
          className: `p-3 flex flex-col gap-2 ${theme === "dark" ? "bg-slate-800 border-t border-slate-700" : "bg-slate-50 border-t border-slate-200"}`
        },
        /* @__PURE__ */ React.createElement("div", { className: "flex justify-end" }, /* @__PURE__ */ React.createElement(
          ...
          /* @__PURE__ */ React.createElement(Icon, { name: "trash" }),
          "איפוס"
        )),
        /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-3 sm:grid-cols-5 gap-2 text-center" }, /* @__PURE__ */ React.createElement("div", { className: `${themeStyles.subtlePanel} p-1 rounded border ${theme === "dark" ? "border-slate-700" : "border-slate-200"}` }, /* @__PURE__ */ React.createElement("div", { className: `text-[9px] ${themeStyles.subtleText}` }, "מרחק"), /* @__PURE__ */ React.createElement("div", { className: "font-bold text-sm" }, stats.dist, "m")), /* @__PURE__ */ React.createElement("div", { className: `${themeStyles.subtlePanel} p-1 rounded border ${theme === "dark" ? "border-slate-700" : "border-slate-200"}` }, /* @__PURE__ */ React.createElement("div", { className: `text-[9px] ${themeStyles.subtleText}` }, "זמן (דקות)"), /* @__PURE__ */ React.createElement("div", { className: `font-bold text-sm ${themeStyles.accent}` }, /* @__PURE__ */ React.createElement(Icon, { name: "clock", size: 10, className: "inline mr-1" }), stats.time)), /* @__PURE__ */ React.createElement("div", { className: `${themeStyles.subtlePanel} p-1 rounded border ${theme === "dark" ? "border-slate-700" : "border-slate-200"}` }, /* @__PURE__ */ React.createElement("div", { className: `text-[9px] ${themeStyles.subtleText}` }, "תמונות"), /* @__PURE__ */ React.createElement("div", { className: `font-bold text-sm ${themeStyles.accent}` }, /* @__PURE__ */ React.createElement(Icon, { name: "camera", size: 10, className: "inline mr-1" }), stats.images)), /* @__PURE__ */ React.createElement("div", { className: `${themeStyles.subtlePanel} p-1 rounded border ${theme === "dark" ? "border-slate-700" : "border-slate-200"}` }, /* @__PURE__ */ React.createElement("div", { className: `text-[9px] ${themeStyles.subtleText}` }, "GSD"), /* @__PURE__ */ React.createElement("div", { className: "font-bold text-sm" }, stats.gsd)), /* @__PURE__ */ React.createElement("div", { className: `${themeStyles.subtlePanel} p-1 rounded border ${theme === "dark" ? "border-slate-700" : "border-slate-200"}` }, /* @__PURE__ */ React.createElement("div", { className: `text-[9px] ${themeStyles.subtleText}` }, "שטח כיסוי"), /* @__PURE__ */ React.createElement("div", { className: `font-bold text-sm ${themeStyles.accent}` }, stats.coverageKm2, ' קמ"ר')))
      )
    ), showPlannerLayout && /* @__PURE__ */ React.createElement(
      MapView,
      {
        className: "flex-1 relative h-full bg-black box-border",
        style: { paddingBottom: mapBottomPadding }
      ...
      /* @__PURE__ */ React.createElement(
        ...
        /* @__PURE__ */ React.createElement(
          "div",
          {
            className: `pointer-events-auto ${isMobile ? "flex flex-row items-start gap-3 w-full" : "flex flex-row items-start gap-3"}`
          },
          /* @__PURE__ */ React.createElement(
            "div",
            {
              className: `${isMobile ? "flex flex-col-reverse gap-2 items-end w-full" : "flex flex-col items-start gap-3"}`
            },
            ENABLE_DOCUMENTATION && documentationOpen && /* @__PURE__ */ React.createElement(
              "div",
              {
              ...
              /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between gap-3" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-2 text-sm font-black text-blue-800" }, /* @__PURE__ */ React.createElement("span", { className: "bg-blue-100 text-blue-700 w-8 h-8 rounded-full flex items-center justify-center" }, /* @__PURE__ */ React.createElement(Icon, { name: "doc", size: 16 })), "כרטיסיית תיעוד"), /* @__PURE__ */ React.createElement(
                "button",
                {
              ...
              /* @__PURE__ */ React.createElement(
                "div",
                {
                  className: `flex flex-col gap-2 rounded-lg border px-3 py-2 text-[11px] ${theme === "dark" ? "bg-slate-900 text-blue-100 border-slate-700" : "bg-blue-50 text-blue-800 border-blue-200"}`
                },
                /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between gap-2" }, /* @__PURE__ */ React.createElement("span", { className: "font-semibold" }, "מצב שמירת התיעוד"), /* @__PURE__ */ React.createElement(
                  "span",
                  {
                    className: `inline-flex items-center gap-1 rounded-full px-2 py-1 text-[10px] ${docStorageMode === "local" ? "bg-green-100 text-green-800" : "bg-slate-100 text-slate-700"}`
                  },
                  docStorageMode === "local" ? "שמירה מקומית" : "סשן בלבד"
                )),
                /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between gap-2" }, /* @__PURE__ */ React.createElement("p", { className: "leading-relaxed" }, docStorageMode === "local" ? "האובייקטים נשמרים במכשיר וממשיכים להיות זמינים גם אחרי רענון." : "האובייקטים נשמרים בסשן הנוכחי בלבד ויימחקו בעת סגירת הטאב."), /* @__PURE__ */ React.createElement(
                  "label",
                  {
                    className: `flex items-center gap-2 text-[11px] font-semibold px-2 py-1 rounded cursor-pointer ${theme === "dark" ? "bg-slate-800 text-blue-100 border border-slate-700" : "bg-white text-blue-800 border border-blue-200"}`
                  },
                  /* @__PURE__ */ React.createElement(
                    "input",
                    {
                      ...
                      onChange: (e) => setDocStorageMode(e.target.checked ? "local" : "session")
                    }
                  ),
                  "שמור מקומית במכשיר"
                ))
              ),
              /* @__PURE__ */ React.createElement("div", { className: "bg-blue-50 border border-blue-200 rounded-xl p-3 space-y-2" }, /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-1 gap-2" }, /* @__PURE__ */ React.createElement(
                "input",
                {
                  type: "text",
                  value: docForm.title,
                  onChange: (e) => setDocForm((prev) => ({
                    ...prev,
              ...
              )), /* @__PURE__ */ React.createElement("div", { className: "flex flex-col gap-2 text-[12px] text-blue-900" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between gap-2" }, /* @__PURE__ */ React.createElement(
                "label",
                {
                  className: `flex items-center gap-2 text-[11px] font-semibold px-2 py-1 rounded ${theme === "dark" ? "bg-slate-900 text-blue-100 border border-slate-700" : "bg-white text-blue-800 border border-blue-200"}`
                },
                /* @__PURE__ */ React.createElement(
                  "input",
                  {
                ...
                "button",
                {
                  ...
                  className: `px-2 py-1 text-[11px] rounded flex items-center gap-1 ${docLocationAllowed ? "bg-blue-600 text-white hover:bg-blue-500" : "bg-blue-100 text-blue-500 cursor-not-allowed"}`
                },
                /* @__PURE__ */ React.createElement(Icon, { name: "gps", size: 12 }),
                " דגום"
              )), /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-2" }, /* @__PURE__ */ React.createElement("span", { className: "bg-blue-100 text-blue-700 rounded-full px-2 py-1 flex items-center gap-1" }, /* @__PURE__ */ React.createElement(Icon, { name: "gps", size: 12 }), " מיקום"), docForm.location ? /* @__PURE__ */ React.createElement("span", { className: "text-[11px] text-blue-700" }, docForm.location.lat.toFixed(5), ",", " ", docForm.location.lng.toFixed(5), " (", docForm.location.accuracy ? `±${Math.round(docForm.location.accuracy)}מ'` : "ללא דיוק", ")") : /* @__PURE__ */ React.createElement("span", { className: "text-[11px] text-blue-700" }, "שמירת מיקום כבויה, נתונים לא יאוחסנו")), /* @__PURE__ */ React.createElement(
                "label",
                {
                  className: `flex items-center justify-between gap-2 cursor-pointer text-[11px] rounded-lg px-3 py-2 border hover:border-blue-400 ${theme === "dark" ? "text-blue-100 bg-slate-900 border-slate-700" : "text-blue-800 bg-white border-blue-200"}`
                },
                /* @__PURE__ */ React.createElement("span", { className: "flex items-center gap-2 font-semibold" }, /* @__PURE__ */ React.createElement(Icon, { name: "upload", size: 14 }), " העלאת תמונות"),
                /* @__PURE__ */ React.createElement(
                  "input",
                  {
                    type: "file",
                ...
                /* @__PURE__ */ React.createElement("span", { className: "text-[10px] text-blue-700" }, docForm.images.length, " נבחרו")
              )), /* @__PURE__ */ React.createElement(
                "button",
                {
                ...
                /* @__PURE__ */ React.createElement(Icon, { name: "mission", size: 16 }),
                " שמור אובייקט"
              )),
              /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-2" }, /* @__PURE__ */ React.createElement(
                "button",
                {
                  ...
                  className: `flex-1 rounded-lg py-2 text-sm font-semibold flex items-center justify-center gap-2 ${theme === "dark" ? "bg-slate-900 text-white hover:bg-slate-800" : "bg-blue-600 text-white hover:bg-blue-500"}`
                },
                /* @__PURE__ */ React.createElement(Icon, { name: "export", size: 16 }),
                " PDF"
              ), /* @__PURE__ */ React.createElement(
                "button",
                {
                  onClick: exportDocumentationShapefile,
                  className: `flex-1 rounded-lg py-2 text-sm font-semibold flex items-center justify-center gap-2 ${theme === "dark" ? "bg-slate-800 text-white border border-slate-600 hover:bg-slate-700" : "bg-slate-100 text-slate-800 border border-slate-300 hover:bg-slate-200"}`
                },
                /* @__PURE__ */ React.createElement(Icon, { name: "map", size: 16 }),
                " Shapefile"
              ), /* @__PURE__ */ React.createElement(
                "button",
                {
                  onClick: clearDocumentationEntries,
                  className: `flex-1 rounded-lg py-2 text-sm font-semibold flex items-center justify-center gap-2 ${theme === "dark" ? "bg-slate-800 text-red-200 border border-slate-600 hover:bg-slate-700" : "bg-red-50 text-red-700 border border-red-200 hover:bg-red-100"}`
                },
                /* @__PURE__ */ React.createElement(Icon, { name: "trash", size: 16 }),
                " נקה תיעוד מקומי"
              )),
              /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-2" }, /* @__PURE__ */ React.createElement(
                "button",
                {
                  ...
                  className: `flex-1 rounded-lg py-2 text-sm font-semibold flex items-center justify-center gap-2 ${theme === "dark" ? "bg-slate-900 text-white border border-slate-700 hover:bg-slate-800" : "bg-white text-blue-800 border border-blue-200 hover:bg-blue-50"}`
                },
                /* @__PURE__ */ React.createElement(Icon, { name: "export", size: 16 }),
                " הורד קובץ פרויקט"
              ), /* @__PURE__ */ React.createElement(
                "button",
                {
                  ...
                  className: `flex-1 rounded-lg py-2 text-sm font-semibold flex items-center justify-center gap-2 ${theme === "dark" ? "bg-blue-800 text-white hover:bg-blue-700" : "bg-green-100 text-green-800 border border-green-200 hover:bg-green-50"}`
                },
                /* @__PURE__ */ React.createElement(Icon, { name: "upload", size: 16 }),
                " שיתוף וואטסאפ/שיתוף מכשיר"
              )),
              /* @__PURE__ */ React.createElement(
                "div",
                {
                  className: `text-[11px] rounded-lg border p-2 ${theme === "dark" ? "bg-slate-900/60 text-slate-200 border-slate-700" : "bg-white text-slate-700 border-blue-200"}`
                },
                "נתוני התיעוד נשמרים אוטומטית בהתאם למצב שנבחר למעלה. שמירת מיקום נשמרת רק אם הפיצ'ר פעיל וניתן לניקוי בכל עת. ניתן גם להוריד קובץ פרויקט או לשתף מידית בוואטסאפ."
              ),
              /* @__PURE__ */ React.createElement("div", { className: "space-y-2 max-h-72 overflow-y-auto custom-scroll" }, docEntries.length === 0 ? /* @__PURE__ */ React.createElement(
                "div",
                {
                  className: `text-[11px] border border-dashed rounded-lg p-3 text-center ${theme === "dark" ? "text-slate-300 border-slate-600" : "text-slate-500 border-slate-300"}`
                },
                "אין אובייקטים מתועדים עדיין. שמור אובייקט כדי להתחיל."
              ) : docEntries.map((entry) => /* @__PURE__ */ React.createElement(
                "div",
                {
                  key: entry.id,
                  className: `rounded-lg p-3 space-y-1 border ${theme === "dark" ? "border-slate-700 bg-slate-800" : "border-slate-200 bg-white/80"}`
                },
                /* @__PURE__ */ React.createElement("div", { className: "flex items-start justify-between gap-2" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement(
                  "div",
                  {
                  ...
                  "div",
                  {
                    className: `text-[10px] rounded px-2 py-1 ${theme === "dark" ? "text-blue-200 bg-blue-900/30 border border-blue-700" : "text-blue-800 bg-blue-50 border border-blue-200"}`
                  },
                ...
                entry.notes && /* @__PURE__ */ React.createElement(
                  "p",
                  {
                    className: `text-[12px] leading-relaxed ${theme === "dark" ? "text-slate-200" : "text-slate-700"}`
                  },
                  entry.notes
                ),
                entry.images?.length > 0 && /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap gap-2 mt-1" }, entry.images.map((img, idx) => /* @__PURE__ */ React.createElement(
                  "img",
                  {
                    key: idx,
                    src: img.dataUrl,
          ...
          /* @__PURE__ */ React.createElement("div", { className: "flex flex-col items-start gap-2" }, ENABLE_MISSION_PLANNING && /* @__PURE__ */ React.createElement(
            DockButton,
            {
              icon: "mission",
              label: "תוכנית טיסה",
